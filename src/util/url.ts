...
export const url =
  process.env.RPC_URL ||
  (process.env.LIVE ? clusterApiUrl(cluster, false) : clusterApiUrl(CLUSTER_DEVNET)); // 'http://localhost:8899');

export const urlTls =
  process.env.RPC_URL ||
  (process.env.LIVE ? clusterApiUrl(cluster, true) : clusterApiUrl(CLUSTER_DEVNET)); //'http://localhost:8899');
